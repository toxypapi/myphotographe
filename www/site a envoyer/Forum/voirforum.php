...
{	
	echo'Vous n\'avez pas l\'autorisation de poster un topic';
	}
?>
<?php
...
Mb.identifiant AS membre_pseudo_createur, post_createur, post_time, Ma.identifiant AS membre_pseudo_last_posteur, post_id FROM forum_topic 
LEFT JOIN validation Mb ON Mb.id = forum_topic.topic_createur
LEFT JOIN forum_post ON forum_topic.topic_last_post = forum_post.post_id
LEFT JOIN validation Ma ON Ma.id = forum_post.post_createur    
...
if ($query->rowCount()>0)
{
        ...
        while ($data=$query->fetch())
        {
                ...
                <td><a href="./voirprofile.php?m='.$data['topic_createur'].'
		...
		$page = ceil($nbr_post / $nombreDeMessagesParPage);
                ...
                <a href="./voirprofile.php?m='.$data['post_createur'].'
                &amp;action=consulter_membre">
...
$query=$db->prepare('SELECT forum_topic.topic_id, topic_titre, topic_createur, topic_vu, topic_post, topic_time, topic_last_post,
Mb.identifiant AS membre_pseudo_createur, post_id, post_createur, post_time, Ma.identifiant AS membre_pseudo_last_posteur FROM forum_topic
LEFT JOIN validation Mb ON Mb.id = forum_topic.topic_createur
LEFT JOIN forum_post ON forum_topic.topic_last_post = forum_post.post_id
LEFT JOIN validation Ma ON Ma.id = forum_post.post_createur   
WHERE topic_genre <> "Annonce" AND forum_topic.forum_id = :forum
...
?>
        ...
        while ($data = $query->fetch())
        {
                ...
                echo'<tr><td><img src="http://uploads.siteduzero.com/files/40001_41000/40792.gif" alt="Message" /></td>
